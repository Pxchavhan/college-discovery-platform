  ...
  return (
    <div className="relative group block h-full">
      <Link 
        ...
        className={`relative rounded-2xl overflow-hidden bg-white/5 backdrop-blur-md border ${isSelected ? 'border-blue-500 shadow-[0_0_20px_rgb(59,130,246,0.2)]' : 'border-white/10 hover:border-blue-500/40'} p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgb(59,130,246,0.12)] flex flex-col h-full block`}
      >
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-tr from-blue-500 to-emerald-400 flex items-center justify-center shadow-md flex-shrink-0">
          ...
          <h3 className="text-lg font-bold text-slate-100 mb-2 leading-snug group-hover:text-blue-400 transition-colors pr-4 line-clamp-2">
            {college.name}
          </h3>
      ...
      {onToggleCompare && (
        <button 
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); onToggleCompare(college.id); }}
          className={`absolute top-5 right-5 z-20 w-7 h-7 rounded-full border flex items-center justify-center transition-all cursor-pointer ${isSelected ? 'bg-blue-500 border-blue-500 text-white shadow-[0_0_12px_rgb(59,130,246,0.4)]' : 'bg-slate-900/80 border-slate-600 text-white hover:border-blue-400 opacity-0 group-hover:opacity-100'}`}
      ...
      {/* Save button */}
      <button 
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (onSaveToggle) onSaveToggle(college.id); }}
        className={`absolute ${onToggleCompare ? 'top-14' : 'top-5'} right-5 z-20 w-7 h-7 rounded-full border flex items-center justify-center transition-all cursor-pointer ${isSaved ? 'bg-emerald-500 border-emerald-500 text-white shadow-md shadow-emerald-500/30' : 'bg-slate-900/80 border-slate-600 text-slate-400 hover:border-emerald-400 hover:text-emerald-400 opacity-0 group-hover:opacity-100'}`}
